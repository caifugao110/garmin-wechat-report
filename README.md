...
1. 打开「健康」App → 浏览 → 体重，确认有最近的测量记录
2. 如果没有，打开「华为运动健康」App → 设置 → 数据权限 → 确保已开启「写入 Apple 健康」，等体脂秤下次称重后数据会自动同步过来
3. 如果你的 iPhone 单位是英制（lb/磅），先去「设置 → 健康 → 单位」改成公制（kg/公斤），避免后续单位换算

#### 步骤 1：创建快捷指令

1. 打开「快捷指令」App，点右上角 **+** 新建
2. 重命名为 **「推送体重到日报」**（名字随意，自己认得出就行）

#### 步骤 2：读取 Apple 健康数据

添加 **「查找健康样本」** 动作，依次添加以下类别（每添加一个类别，动作会自动增加一行并生成一个"样本变量"）：

| 类别（中文） | 类别（英文） | 建议变量名 | 单位 | 备注 |
|---|---|---|---|---|
| 体重 | Body Mass | 体重 | kg | 必填 |
| 体脂百分比 | Body Fat Percentage | 体脂率 | % | 可选 |
| 身体质量指数 | Body Mass Index | BMI | 无 | 可选 |
| 去脂体重 | Lean Body Mass | 肌肉量 | kg | 可选 |

每个类别都设置为：
- **样本**：最新（Latest）
- **排序**：按日期（Date）

> ⚠️ **单位换算**：如果你的 iPhone 健康数据单位是 lb（磅），需要加「计算」动作做换算：`体重 / 2.2046 = 体重kg`，后续用换算后的变量。体脂率和 BMI 无量纲，不需要换算。

#### 步骤 3：构造 JSON 请求体

添加 **「词典」** 动作，类型选 **JSON**，按以下结构填写。字段值通过点 "+" → "选择变量" 引用上一步「查找健康样本」的输出变量：
...
> 💡 变量名随你怎么取，**key 名必须完全一致**（`weight` / `bodyFatRate` / `bmi` / `muscleMass`），服务端靠这些 key 识别字段。值可以是数字或字符串，服务端两种都接受。

#### 步骤 4：POST 到 SCF webhook
...
如果返回 `"status":"error"`，检查：
- 「查找健康样本」类别是否真的有数据（先去健康 App 手动称重测试）
- 词典 key 名是否写错
- 单位是否需要换算（如果数值偏大 2.2 倍就是磅没转公斤）

#### 步骤 6：设置自动化定时推送

1. 在快捷指令详情页，点底部 **「在共享表单中使用」** 旁边的 ⓘ → 开启 **「作为快捷指令自动化」**
2. 或者直接去「自动化」Tab → 右上角 + → 创建「个人自动化」→ 触发条件选 **「特定时间」**（建议 07:20，早于日报 07:40）
3. 动作选「运行快捷指令」→ 选刚才创建的「推送体重到日报」
4. 不要开启「运行前询问」（每天自动跑不需要确认）

> iOS 后台策略限制：快捷指令自动化不保证**精确到秒**执行，但每天 07:40 前一定会跑。如果当天没推送（比如手机没联网），解锁手机后一般会补跑。
